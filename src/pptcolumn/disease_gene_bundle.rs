...
//! We bundle this together to deal with these five columns of the template, which are invariant for any one template.
//!
//!
...
impl DiseaseGeneBundle {
    ...
    pub fn new_from_str(
        ...
        let disease_tid =
            TermId::from_str(disease_id).map_err(|e| Error::termid_parse_error(disease_id))?;
...
mod tests {
    ...
    fn test_disease_name() {
        ...
        for test in tests {
            ...
            let result = DiseaseGeneBundle::new(&disease_id, test.0, &hgnc_id, "FBN1", "NM_0123.1");
            if test.1 {
                assert!(result.is_ok());
            } else {
