...
use crate::age::gestational_age::GestationalAgeValidator;
...
pub static ONSET_TERM_DICT: Lazy<HashMap<String, SimpleMinimalTerm>> = Lazy::new(|| {
    let mut onset_d: HashMap<String, SimpleMinimalTerm> = HashMap::new();
    let onset_terms = 
    ...
    for tuple in onset_terms {
        let tid = TermId::from_str(tuple.0).unwrap();
        onset_d.insert(tuple.1.to_string(), SimpleMinimalTerm::new(tid, tuple.1, vec![], false));
    } 
    onset_d
});
