...
fn check_valid_curie(s: &str) -> Result<bool> {
    if s.is_empty() {
        ...
        if s.chars().any(|c| c.is_whitespace()) {
            ...
            return Err(Error::CurieError { msg: format!("Invalid CURIE with no prefix: '{}'", s)});
        } else if  pos == s.len() - 1 {
            return Err(Error::CurieError { msg: format!("Invalid CURIE with no suffix: '{}'", s)});
        } else if let Some((_prefix, suffix)) = s.split_once(':') {
            if ! suffix.chars().all(char::is_numeric) {
                return Err(Error::CurieError { msg: format!("Invalid CURIE with non-digit characters in suffix: '{}'", s)});
            }
        } 
    } else {
        return Err(Error::CurieError { msg: format!("Invalid CURIE with no colon: '{}'", s)});
    }
    Ok(true)
}
...
impl Curie {
    ...
    pub fn new_disease_id(value: &str) -> Result<Self> {
        ...
        if valid_curie.is_err() {
            return Err(Error::DiseaseIdError { msg: format!("Invalid disease identifier: {}", valid_curie.err().unwrap())});
        } else if ! ( value.starts_with("OMIM") || value.starts_with("MONDO") ) {
            return Err(Error::DiseaseIdError { msg:format!("Disease id has invalid prefix: '{}'", value)});
        } else {
            return Ok(Curie { curie_value: value.to_string()});
        }
    }
    ...
    pub fn new_hgnc_id(value: &str) -> Result<Self> {
        let valid_curie = check_valid_curie(value);
        if valid_curie.is_err() {
            return Err(Error::HgncError { msg: format!("Invalid HGNC identifier: {}", valid_curie.err().unwrap())});
        } else if ! value.starts_with("HGNC") {
            return Err(Error::HgncError { msg:  format!("HNGC id has invalid prefix: '{}'", value)});
        } else {
            return Ok(Curie { curie_value: value.to_string()});
        }
    }
