...
impl CohortBuilder {
    pub fn new(disease: DiseaseData, hpo_headers: Vec<HpoTermDuplet>, acronym: &str) -> Self {
        Self { disease, hpo_headers, rows: vec![], acronym: acronym.to_string() }
    }
    ...
    /// Force every row's value at `col_idx` to `na`, regardless of what
    /// the row builder originally set there.
    pub fn with_na_column(mut self, col_idx: usize) -> Self {
        for row in self.rows.iter_mut() {
            row.hpo_data[col_idx] = CellValue::na();
        }
        self
    }

    pub fn build(self) -> CohortData {
        CohortData {
            cohort_type: CohortType::Mendelian,
            disease_list: vec![self.disease],
            hpo_headers: self.hpo_headers,
            rows: self.rows,
            ...
            cohort_acronym: Some(self.acronym),
...
pub fn cohort_with_na_column(
    ...
    // We use three HPO terms because individual_1 and individual_2 have exactly 3 observations
    CohortBuilder::new(disease_data, hpo_term_pool[0..3].to_vec(), "NA_COHORT")
        .with_rows(vec![individual_1, individual_2])
        .with_na_column(na_col_idx)
        .build()
}
