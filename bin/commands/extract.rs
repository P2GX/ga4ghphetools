...
fn process_cohort_file(input_file: &PathBuf, output_dir: &Path, hpo: Arc<FullCsrOntology>) -> Result<(), String> {
    ...
    let msg = ga4ghphetools::ppkt::write_phenopackets(cohort, output_dir.to_path_buf(), orcid, hpo.clone()).map_err(|e|e.to_string())?;
    println!("{}", msg);
...
fn process_cohort_dir(input_dir: &Path, output_dir: &Path, hpo: Arc<FullCsrOntology>) -> Result<(), String> {
    ...
    for entry in entries.flatten() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            process_cohort_file(&path, output_dir, hpo.clone())?;
        }
    }
